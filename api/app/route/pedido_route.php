...
$app->group('/pedido/', function () {
    ...
    $this->get('detallePedido/{id}', function ($req, $res, $args) {
        return $res->withHeader('Content-type', 'application/json')
                   ->write(
                     json_encode($this->model->pedido->obtenerDetalle($args['id']))
                   );
    });
    ...
    $this->put('actualizaEstado/{id}', function ($req, $res, $args) {
    ...
    $this->post('guardar', function ($req, $res, $args) {
        $data = $req->getParsedBody();
        $r = PedidoValidation::validate($data, true);
